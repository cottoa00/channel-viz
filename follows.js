(function ( $ ){
	...
	*/
	...
	var defaultKey		= '87ynD20hZioKhBrsFEywmxEVX7nBI2c0794RW2pR6y9c2OpI', // Unique master Xively API key to be used as a default
		defaultFeeds	= ['1725370504!0'], // Comma separated array of Xively Feed ID numbers
		applicationName	= 'My Home Electricity Monitor', // Replaces Xively logo in the header
...
//					if(duration == '90days') diff = 7776000000;
					...
					if(updated.getTime() > then.getTime()) {
						if(datastreamIds && datastreamIds != '' && datastreamIds.indexOf(datastream.id) >= 0) {
							xively.datastream.history(feedId, datastream.id, {duration: duration, interval: interval, limit: 1000}, function(datastreamData) {
								...
								$('#feed-' + feedId + ' .datastreams .datastream-' + datastream.id + ' .datastream-name').html('Channel ' + datastream.id + tagsHtml);
								...
								if(datastream.unit) {
									if(datastream.unit.symbol) {
										...
										$('#feed-' + feedId + ' .datastreams .datastream-' + datastream.id + ' .datastream-value').html(datastream.current_value + ' W');
									}
								} else {
									$('#feed-' + feedId + ' .datastreams .datastream-' + datastream.id + ' .datastream-value').html(datastream.current_value);
								}
